...
# ----------------------------------------------------------------------------
# Equipment is now sourced from the WalkScape tools API, not the wiki.
# The live path (run by scrape_all.py) is:
#     util/scrapers/update_from_api.py       -> ingest_gear_from_api.py -> equipment.py
#     util/scrapers/fix_equipment_uuids.py   -> resolve real item UUIDs
...
# Aliases for item finding category names that don't match exactly
# Maps wiki drop text (lowercased) → ItemFindingCategory attribute name
ITEM_FINDING_ALIASES = {
    'skilling chest': 'SKILL_CHEST',
    'skill chest': 'SKILL_CHEST',
    'rough gem': 'RANDOM_GEM',
    'random gem': 'RANDOM_GEM',
    'sea shell': 'SEA_SHELLS',
    'sea shells': 'SEA_SHELLS',
    'fibrous plant': 'FIBROUS_PLANT',
    'bird nest': 'BIRD_NEST',
}
...
def parse_item_page(html_content, item_name):
    ...
    if infobox:
        for row in infobox.find_all('tr'):
                ...
                # Extract keywords
                # Match the real "Keyword" row only (NOT "Search Keyword",
                # whose value is a skill name and would pollute keywords).
                header_norm = header_text.strip().rstrip(':').strip().lower()
                if header_norm in ('keyword', 'keywords'):
                    ...
                    if keyword_cell:
                        ...
                        for link in keyword_links:
                            ...
                            if not keyword_text:
                                # Image-only keyword link (e.g. the "Upgradeable"
                                # keyword renders as just an icon with no link
                                # text) -- fall back to the link title / img alt.
                                keyword_text = (link.get('title') or '').strip()
                                if not keyword_text:
                                    kw_img = link.find('img')
                                    if kw_img and kw_img.get('alt'):
                                        keyword_text = kw_img.get('alt').strip()
                            if keyword_text and not keyword_text.endswith('.svg'):
                                # A keyword often has both an icon link and a
                                # text link in the same cell -> dedupe (case-
                                # insensitive, keep first-seen casing).
                                if not any(keyword_text.lower() == k.lower()
                                           for k in item_data['keywords']):
    ...
    if requirement_section:
        ...
        if req_list:
            for li in req_list.find_all('li', recursive=False):
                ...
                rep_match = re.search(r'Have\s*[\(\[](\d+)[\)\]]\s*(.+?)\s+faction\s+reputation', req_text, re.IGNORECASE)
                if rep_match:
                ...
                # Parse category level percentage requirement: "Have 35% towards maximum Artisan level [206]"
                # Categories: Artisan, Gathering, Utility
                # The [206] is just the wiki showing the computed total — we only need the percent and category
                cat_pct_match = re.search(r'Have\s+(\d+)%\s+towards\s+maximum\s+(\w+)\s+level\s+[\(\[](\d+)[\)\]]', req_text, re.IGNORECASE)
                if cat_pct_match:
                    percent = int(cat_pct_match.group(1))
                    category = cat_pct_match.group(2).strip().lower()
                    requirements.append({
                ...
                if len(cells) >= 3:
                    ...
                    if quality_img:
                        ...
                        if quality in QUALITY_LEVELS:
                            ...
                            # Parse lines, looking ahead for location/skill/activity requirements
                            i = 0
                            while i < len(lines):
                                line = lines[i]
                                line_lower = line.lower()

                                # Skip pure qualifier lines (skill restriction, location, etc.)
                                # that were already consumed as lookahead on a previous iteration
                                location_req = None
                                skill_override = None

                                # Look ahead for qualifiers on the next line(s)
                                next_i = i + 1
                                while next_i < min(i + 3, len(lines)):
                                    next_line = lines[next_i]
                                    next_line_lower = next_line.lower()

                                    if 'while in' in next_line_lower or 'not in an' in next_line_lower:
                                        is_negated = '(not)' in next_line_lower or 'not in an' in next_line_lower
                                        loc_match = re.search(r'(?:while in (?:the )?|not in an\s+)([^.]+?)(?:\s+(?:location|area))?\.?$', next_line_lower)
                                        if loc_match:
                                    ...
                                    if 'while doing' in next_line_lower:
                                        # Only treat as skill restriction if no stat value on the line
                                        has_stat_value = bool(re.search(r'[+-]?\d+(?:\.\d+)?%?\s+\w', next_line))
                                        if not has_stat_value:
                                            extracted_skill = extract_skill_from_text(next_line)
                                            if extracted_skill and extracted_skill != 'global':
                                                skill_override = extracted_skill
                                            next_i += 1
                                            continue

                                    break  # Not a qualifier line — stop looking ahead

                                # Check if current line has "while doing" for an activity (not skill)
                                is_activity = is_activity_stat(line)

                                # Parse the stat line with location context and optional skill override
                                parse_stat_line_with_location(line, quality_item_data, location_req, skill_override=skill_override)
                                i = next_i
            ...
            if has_achievement_points:
                ...
                while i < len(lines):
                    ...
                    while next_i < min(i + 4, len(lines)):
                        ...
                        if 'while doing' in next_line_lower:
                            ...
                            if is_activity_stat(next_line):
                                ...
                                # Only treat as skill restriction if this line is ONLY a
                                # "while doing" qualifier (no stat value on the same line).
                                # Lines like "+4% DR While doing Fishing" are separate stats,
                                # not skill restrictions for the previous stat.
                                has_stat_value = bool(re.search(r'[+-]?\d+(?:\.\d+)?%?\s+\w', next_line))
                                if not has_stat_value:
                                    # Pure skill restriction line (e.g., "While doing Gathering skills.")
                                    extracted_skill = extract_skill_from_text(next_line)
                                    if extracted_skill and extracted_skill != 'global':
                                        skill_level_req = {
                                    ...
                                    # This is a new stat line, not a qualifier — stop looking
                                    break
                        ...
                        if 'requires' in next_line_lower and 'equipped' in next_line_lower:
                            # Set piece requirement: "Requires [5] Proper gear equipped." or "Requires [5] unique Proper gear equipped."
                        ...
                        if 'walk a total amount of steps dedicated to travel' in next_line_lower:
                            # Travel steps requirement: "Walk a total amount of steps dedicated to travel. [125,000]"
                            travel_match = re.search(r'[\[\(]([\d,]+)[\]\)]', next_line)
                            if travel_match:
                                threshold = int(travel_match.group(1).replace(',', ''))
                                skill_level_req = {
                                    'type': 'travel_steps',
                                    'threshold': threshold
                                }
                    ...
                    if skill_level_req or item_ownership_req:
                        ...
                        if gate_req.get('type') == 'skill_restriction':
                            # Skill restriction is not a gate — it's the skill key for the stat
                            parse_stat_line_with_location(line, item_data, location_req, skill_override=gate_req['skill'])
                        else:
                            # Parse as gated stat
...
def parse_gated_stat_line(text, item_data, location_req=None, skill_level_req=None):
    ...
    # "X experience on any action" stats apply on ALL actions, so store under 'global' skill
    if stat_name.endswith('_base_xp'):
        skill = 'global'
    ...
    if skill_level_req:
        ...
        if req_type == 'skill_level':
        ...
        elif req_type == 'travel_steps':
            # Travel steps requirement (e.g., "Walk a total amount of steps dedicated to travel. [125,000]")
            # Structure: {'travel_steps': {threshold: {skill: {location: {stat: value}}}}}
            gate_type = 'travel_steps'
            threshold = skill_level_req['threshold']
            ...
            item_data['gated_stats'][gate_type][threshold][skill][location_key][final_stat_name] = final_value
            return  # Early return for travel_steps gates
        else:
...
def parse_item_drop_line(text, item_data, location_req=None):
    ...
    # Strip common prefixes like "basic" that the wiki adds before category names
    stripped_name_lower = re.sub(r'^(basic|random)\s+', '', item_name_lower).strip()
    ...
    if item_name_lower in DIRECT_ITEM_DROPS:
    ...
    elif stripped_name_lower in ITEM_FINDING_ALIASES:
        # Use explicit alias mapping
        stat_name = f"ItemFindingCategory.{ITEM_FINDING_ALIASES[stripped_name_lower]}"
    else:
        ...
        try:
            ...
            # 3. Stripped name matches category name (e.g., "basic fibrous plant" → "fibrous plant" == "Fibrous Plant")
            ...
            for attr_name in dir(ItemFindingCategory):
                ...
                if hasattr(category, 'name'):
                    ...
                    # Check if category name matches (original or stripped)
                    if cat_name_lower == item_name_lower or cat_name_lower == stripped_name_lower:
...
def parse_stat_line_with_location(text, item_data, location_req=None, skill_override=None):
    """Parse a stat line with explicit location requirement and optional skill override"""
    text_lower = text.lower()
    ...
    # Use skill_override if provided (e.g., from "While doing Gathering skills" on a separate line)
    skill = skill_override if skill_override else extract_skill_from_text(text)
    ...
    if stat_name:
        ...
        if stat_name.endswith('_base_xp'):
            ...
            skill = 'global'
            # Re-initialize structures for global skill if needed
            if skill not in item_data['skill_stats']:
                item_data['skill_stats'][skill] = {}
            if location_key not in item_data['skill_stats'][skill]:
                item_data['skill_stats'][skill][location_key] = {}
            # Clean up empty dict left behind from original skill initialization
            if original_skill != skill and original_skill in item_data['skill_stats']:
                if location_key in item_data['skill_stats'][original_skill] and not item_data['skill_stats'][original_skill][location_key]:
                    del item_data['skill_stats'][original_skill][location_key]
                if not item_data['skill_stats'][original_skill]:
                    del item_data['skill_stats'][original_skill]
...
def generate_equipment_py(items):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        '        """Deep-merge two nested stat dicts. Overlay values override base at leaf level."""',
        ...
        '        return ItemInstance(f"{self.name} ({quality})", self.uuid, stats, self.slot, self.keywords, value, rarity=None, requirements=self.requirements)',
        '    ',
        ]
        ...
        for quality in QUALITY_LEVELS:
            ...
            f'        return self._get_quality("{quality}")',
            '    ',
            ])
        ...
        # Backwards-compatibility aliases for renamed items
        # Maps old_enum_name -> new_enum_name
        ALIASES = {
            'CAMOUFLAGE_CAPE': 'LEAF_CAPE',
        }
        
        # Only emit aliases for items that actually exist in this generation
        item_consts = set()
        for item in items:
            item_consts.add(item['name'].upper().replace(' ', '_').replace("'", '').replace('-', '_'))
        
        alias_lines = []
        for old_name, new_name in sorted(ALIASES.items()):
            if new_name in item_consts:
                alias_lines.append(f'    {old_name} = {new_name}')
        
        if alias_lines:
            lines.append('    # Backwards-compatibility aliases for renamed items')
...
def main():
    ...
    if SCAN_FOLDER_FOR_NEW_ITEMS:
        ...
        if folder_items:
            # Build set of existing item names (case-insensitive to avoid duplicates
            # from filename casing differences, e.g. "Shiny ring" vs "Shiny Ring")
            existing_names_lower = {name.lower() for name, url, uuid in equipment_links}
            ...
            for item in folder_items:
                if item['name'].lower() not in existing_names_lower:
                    ...
                    print(f"  ⊘ Skipped duplicate (case-insensitive): {item['name']}")
