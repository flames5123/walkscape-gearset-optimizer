...
def parse_services():
    ...
    # Apply hardcoded overrides for known coin-gated services. The wiki does
    # not surface coin-purchase requirements (the Tools API exposes them as
    # `haveCoins`), so we inject them post-parse to keep services.py in sync
    # with services_from_api.py for the unlock logic.
    apply_coin_gate_overrides(services)
    
    return services

# Hardcoded coin requirements for services where the wiki page omits the
# 'haveCoins' requirement. Keys are (service_name_lower, location_name_lower)
# tuples, values are required coin counts. Mirrors the API ingest output in
# services_from_api.py — keep in sync if new coin-gated services are added.
COIN_GATED_SERVICES = {
    ('erdwiss trinketry factory', 'blackspell harbour'): 77777,
}

def apply_coin_gate_overrides(services):
    """Inject `{'type': 'coins', 'coins': N}` access entries into services
    that the wiki doesn't list as coin-gated. Modifies `services` in-place.

    Idempotent: skips services that already have a coin access entry.
    """
    for svc in services:
        name_l = (svc.get('name') or '').strip().lower()
        loc_l = (svc.get('location_name') or '').strip().lower()
        coins = COIN_GATED_SERVICES.get((name_l, loc_l))
        if not coins:
            continue
        req = svc.setdefault('requirement', {})
        access = req.setdefault('access', [])
        if any(isinstance(a, dict) and a.get('type') == 'coins' for a in access):
            continue
        access.append({'type': 'coins', 'coins': coins})
