...
class Sorting(Enum):
    ...
    def __new__(cls, sort_direction, metric_key, display_name):
    ...
    # Enum values: (sort_direction, metric_key, display_name)
    # Activity-specific sorting
    TOTAL_CRAFTS = ("+target_quantity", "target_quantity", "Total Crafts")
    REWARD_ROLLS_PER_STEP = ("+reward_rolls_per_step", "reward_rolls_per_step", "Reward Rolls/Step")
    STEPS_PER_REWARD_ROLL = ("-steps_per_reward_roll", "steps_per_reward_roll", "Steps/Reward Roll")
    TOTAL_STEPS = ("-steps_for_target", "steps_for_target", "Total Steps")
    XP_PER_STEP = ("+xp_per_step", "primary_xp_per_step", "XP/Step")
    TOTAL_XP = ("+xp_for_target", "xp_for_target", "Total XP")
    EXPECTED_STEPS_PER_ACTION = ("-expected_steps_per_action", "expected_steps_per_action", "Steps/Action")
    FINE_MATERIAL_FINDING = ("+fine_material_finding_pct", "fine_material_finding_pct", "Fine Material Finding %")
    
    # Crafting-specific sorting
    MATERIALS = ("-materials_for_target", "materials_for_target", "Materials")
    TOTAL_CRAFTS_FOR_TARGET = ("-total_crafts", "total_crafts", "Total Crafts")
    STEPS_PER_CRAFT = ("-current_steps", "current_steps", "Steps/Craft")
    MATERIALS_PER_CRAFT = ("-materials_per_craft", "materials_per_craft", "Materials/Craft")
    QUALITY_OUTCOME = ("+quality_outcome", "quality_outcome", "Quality Outcome")
    PERFECT_PERCENTAGE = ("+perfect_percentage", "perfect_percentage", "Perfect %")
