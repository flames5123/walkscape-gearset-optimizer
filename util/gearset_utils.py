...
class Gearset:
    ...
    def __init__(self, export_string: str = None):
        ...
        self._tool4 = None
        self._tool5 = None
        # Pet + consumable slots. These are NOT equipment Items — pets resolve
        # via util.autogenerated.pets (PetInfo, level-scoped) and consumables via
        # util.autogenerated.consumables (ConsumableItem). Both are StatsMixins,
        # so they aggregate through get_stats_for_skill like gear. Modeling them
        # here lets the travel stats endpoint (and, later, the travel optimizer)
        # account for pet WE (e.g. Reindeer +4% agility) and food WE (e.g. Bagel
        # +5% WE) that the gear-only path previously dropped.
        self._pet = None
    ...
    def _load_from_export(self, export_string: str):
        ...
        for item_data in data['items']:
            ...
            # Pet + consumable slots resolve via their own models, NOT Item.by_uuid
            # (which only knows equipment and would raise/return None). Mirrors the
            # canonical resolver used by the activity-stats path in ui/app.py.
            if slot_type == 'pet':
                try:
                    from util.autogenerated.pets import PETS_BY_NAME, PETS_BY_RARE_EXPORT_NAME
                    species = str(item_json.get('species') or item_json.get('id') or '').strip()
                    level = int(item_json.get('level') or 0)
                    if species and level > 0:
                        pet_levels = (PETS_BY_NAME.get(species.capitalize())
                                      or PETS_BY_RARE_EXPORT_NAME.get(species.lower()))
                        if pet_levels:
            ...
            if slot_type == 'consumable':
                try:
                    ...
                    export_name = str(item_json.get('id') or item_json.get('name') or '').strip()
                    is_fine = bool(item_json.get('is_fine'))
                    # Defensive: an id may already carry the _fine suffix.
                    if export_name.lower().endswith('_fine'):
                        export_name = export_name[:-5]
                        is_fine = True
                    cons = None
                    if export_name:
                        if is_fine:
                            cons = (Consumable.by_export_name(export_name + '_fine')
                                    or Consumable.by_export_name(export_name))
                        else:
                            cons = Consumable.by_export_name(export_name)
                    if cons is not None:
                        self._consumable = cons
                except Exception:
                    pass
                continue
    ...
    @property
    def consumable(self): return self._consumable
    
    def get_all_items(self, include_pet_consumable: bool = False) -> List[Tuple[str, object]]:
        """Get all equipped items as (slot_name, item) tuples.

        By default returns only gear/rings/tools, preserving behavior for the
        many callers (set-piece counting, activity/crafting/service stats) that
        expect equipment-only. Pass include_pet_consumable=True (travel stats /
        travel optimizer) to also include the pet and consumable slots so their
        stats (e.g. Reindeer +4% agility WE, Bagel +5% WE) aggregate like gear.
        """
        items = []
        slots = ['head', 'cape', 'back', 'chest', 'primary', 'secondary', 
                'hands', 'legs', 'neck', 'feet', 'ring1', 'ring2',
                'tool0', 'tool1', 'tool2', 'tool3', 'tool4', 'tool5']
        if include_pet_consumable:
            slots = slots + ['pet', 'consumable']
...
def aggregate_gearset_stats(
    ...
    Args:
        ...
        activity: Activity object or name string, or None. When provided,
            activity-gated stats (gated_stats['activity'][<name>]) are
            applied ONLY for that activity. Leave None for recipe/craft/
            travel paths so activity-scoped bonuses don't leak globally.
            (2026-06-18 jwbail: previously never threaded, so activity-only
            bonuses like the Zippy kicksled's -5 steps on Sledding were
            silently dropped. The resolver in util/stats_mixin._add_gated_stats
            gates by exact activity name, so passing it here cannot apply the
            bonus to any other activity.)
    ...
    if include_level_bonus:
        ...
        if is_travel:
            # Travel: 0.5% per agility level (no cap). Level 1 = no boost
            # (matches the in-game UI and ui/static/js/components/
            # combined-stats-section.js which uses `(charLevel - 1) * 0.5`).
            # The wiki example at
            # https://wiki.walkscape.app/wiki/Traveling_(Mechanics) shows
            # `30 × 0.005` rather than `(30 - 1) × 0.005` — that example
            # is slightly off; the game treats level 1 as the baseline.
...
def calculate_gear_stats(gearset_export: str, skill: str, location=None, return_breakdown: bool = False):
    """
...
def encode_gearset(gearset_dict: Dict[str, object]) -> str:
    ...
    for slot_name, slot_type in slot_type_map.items():
        if slot_name in gearset_dict and gearset_dict[slot_name] is not None:
            ...
            # Extract quality from item name; fall back to item.rarity for non-crafted items
            quality = None
            for quality_name, quality_value in QUALITY_NAME_TO_EXPORT.items():
                if f'({quality_name})' in item.name:
                    ...
                    break
            if quality is None:
                # Non-crafted item — use its rarity directly (already in export format)
                quality = getattr(item, 'rarity', 'common') or 'common'
    ...
    # Generic items (uuid starts with 'generic::') can't be decoded from the catalog,
    # so store them in a custom extension field for round-trip support
    generic_slots = {}
    for slot_name, item in gearset_dict.items():
        if item is not None and hasattr(item, 'uuid') and str(item.uuid).startswith('generic::'):
            generic_slots[slot_name] = {
                'itemId': item.uuid,
                'uuid': item.uuid,
                'name': item.name,
                'icon_path': None,
                ...
                'rarity': getattr(item, 'rarity', None) or 'common',
                'slot': getattr(item, 'slot', None),
                'keywords': list(getattr(item, 'keywords', [])),
                'is_generic': True,
                'stats': {},
            }
    ...
    if generic_slots:
        gearset_json['generic_slots'] = generic_slots
