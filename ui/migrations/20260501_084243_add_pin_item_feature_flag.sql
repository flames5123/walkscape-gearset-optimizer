-- Register the `pin_item` feature flag for per-session gating.
--
...
-- in. The owner's UUID is also hardcoded in database.py
-- (_PIN_ITEM_ALWAYS_ENABLED) for dev/testing access. No rows are inserted
-- globally — enablement happens via the admin UI or the hardcoded list.
--
