...
RECIPE = Recipe.IRON_SICKLE  # Use Recipe.RECIPE_NAME
SERVICE = Service.BASIC_WORKSHOP_FRUSENHOLM  # Use Service.SERVICE_NAME_LOCATION
GEARSET_EXPORT = ""
...
MATERIAL_BUDGET = 100  # Or specify materials available and calculate max items (set to None to use TARGET_QUANTITY)

# Advanced options
USE_ITERATIVE_CALCULATION = False  # Account for level-ups during crafting (more accurate)
...
def is_quality_item(recipe_name: str) -> bool:
    ...
    try:
        ...
        # Recipe names are like "Craft an iron sickle", "Bake a bagel", etc.
        # Extract the item name by removing common prefixes
        item_name = recipe_name
        for prefix in ['Craft an ', 'Craft a ', 'Bake a ', 'Bake an ', 'Cook a ', 'Cook an ', 'Make a ', 'Make an ']:
            if item_name.startswith(prefix):
                item_name = item_name[len(prefix):]
                break
        
        enum_name = item_name.upper().replace(' ', '_').replace('-', '_')
...
def print_craft_stats(stats, show_item_breakdown=True):
    ...
    # Display crafting odds table for quality items (similar to optimize_craft_gearsets.py)
    if stats['is_quality_item']:
        quality_result = calculate_quality_weights(stats['level_required'], stats['total_qo'])
        
        print(f"CRAFTING ODDS (Recipe Level: {stats['level_required']}, QO: {stats['total_qo']:.0f}):")
        print(f"  {'Quality':<12} {'Chance':>10} {'Avg. Crafts':>14} {'Avg. Materials':>18}")
        print(f"  {'-'*12} {'-'*10} {'-'*14} {'-'*18}")
        
        # Display each quality tier
        quality_order = ['Normal', 'Good', 'Great', 'Excellent', 'Perfect', 'Eternal']
        for quality in quality_order:
            percentage = quality_result['percentages'][quality]
            avg_crafts = 100.0 / percentage if percentage > 0 else float('inf')
            avg_materials = avg_crafts / stats['crafts_per_material'] if percentage > 0 else float('inf')
            
            # Highlight target quality
            marker = "→" if quality == stats.get('target_quality') else " "
            
            if avg_crafts == float('inf'):
                print(f"{marker} {quality:<12} {percentage:>9.2f}% {'N/A':>14} {'N/A':>18}")
            else:
                print(f"{marker} {quality:<12} {percentage:>9.2f}% {avg_crafts:>14.1f} {avg_materials:>18.1f}")
...
if __name__ == '__main__':
    ...
    # Use default gearset from my_config if not specified
    gearset_to_use = GEARSET_EXPORT
    if gearset_to_use is None:
        try:
            character = get_character()
            gearset_to_use = character.gearset_export
            print(f"\nUsing default gearset from my_config.py")
        except:
            print("\nPlease configure the tool by editing the variables at the top of this file:")
            print("  - RECIPE: Recipe object (e.g., Recipe.IRON_SICKLE)")
            print("  - SERVICE: Service object (e.g., Service.BASIC_FORGE_KALLAHEIM)")
            print("  - GEARSET_EXPORT: Your gearset export string (or set to None to use default from my_config)")
            print("\nCharacter export is loaded from my_config.py")
